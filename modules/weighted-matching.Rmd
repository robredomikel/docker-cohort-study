...
raw_df <- subset(raw_df, select = -c(trimmed_languages))ß
...
df$MS.NonMS = factor(df$MS.NonMS)
library(lattice)
...
```{r}
exactMatching <- matchit(MS.NonMS ~ main_language + n_languages, data = df, method = "exact")
